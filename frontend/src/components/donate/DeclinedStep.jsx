 ...
 */
export default function DeclinedStep({ reason, locale, onRetry, onCancel }) {
  ...
  return (
    <div className="ledger-tape p-8 w-full max-w-sm text-center">
      ...
      <h2 className="font-display text-2xl mb-1">{isOffline ? t('donate.offline') : t('donate.declined')}</h2>
      <p className="text-ink/60 mb-2">{isOffline ? t('donate.offlineHelp') : t('donate.declinedHelp')}</p>
      {reason && !isOffline && <p className="text-xs text-ink/40 mb-6 tabular">({reason})</p>}
